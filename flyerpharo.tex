...
in just a few minutes In Pharo, the concepts are \emph{very consistent}:
\begin{itemize}
...
\emph{image} in which all objects live and can be modified. An image
is like a smart cache. The state of an image can be saved and reloaded
later to get all your objects and tools back. Image-based development
feels really different from file-based development.
...
The example sends the message \code{do:} to an array.
This evaluates the block once for each element, passing it via the \code{each} parameter. As a result, the transcript displays the text \code{Hello~World!}.
