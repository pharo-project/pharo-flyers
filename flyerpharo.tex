...
results in a new collection to which \textcolor{darkRed}{3}
...
\item classes are object too.
\item there is single inheritance between classes.
\item traits are group of methods that can be reused orthogonally to inheritance.
\item instance variables are protected.
\item methods are public. 
\item blocks are lexical closure a.k.a. anonymous methods.
\item all computation happens via message sends (and variable assignment).
\end{itemize}

There is no: type declaration, primitive object, generic, modifier, operator, inner classes, 
static method.
