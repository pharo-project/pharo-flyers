...
When we send a message to an object (the \emph{receiver}), the corresponding method is selected and executed, and the method answers an object. Messages syntax mimics natural languages, with a subject, a verb, and complements.
...
The whole message cascade value is the value of the last message sent (the symbol
\textcolor{string}{\code{\#g}}). To return the receiver of the
message cascade instead (\ie the collection), send
...
classes, no constructors, and no static methods. There are not needed!
